...
\fancyhf{}
\rfoot{\thepage}
\lhead{Tarea 2 - Ingeniería de Software}
...
\subsection*{Persistencia y datos iniciales}
La configuración de base de datos reside en \texttt{application.properties}, con credenciales sobreescribibles por variables de entorno. El archivo \texttt{data.sql} carga datos de inicio (muebles, variaciones y cotizaciones) para validar rápidamente el flujo en local o con Docker Compose.
...
\begin{itemize}
    ...
    \item \textbf{Service Layer}: los servicios encapsulan reglas de negocio (validación de stock, estados y pertenencia de variaciones) y mantienen los controladores delgados.
...
Todas las pruebas se ejecutan con una base H2 aislada y se disparan mediante \texttt{./mvnw test}, generando reportes en \texttt{target/surefire-reports/}.
...
\begin{itemize}
    \item \textbf{Requisitos}: Docker + Docker Compose; o bien JDK 21, Maven Wrapper (\texttt{./mvnw}) y MySQL disponible en \texttt{localhost:3306} con el esquema \texttt{muebles\_db}.
    \item \textbf{Docker Compose (recomendado)}: levanta la app y MySQL en segundos.
    \begin{enumerate}[label=\arabic*.]
        \item Construir y levantar: \texttt{docker compose build} seguido de \texttt{docker compose up -d}.
        \item Ver logs de arranque: \texttt{docker compose logs app -f}.
        \item Servicios expuestos: backend en \texttt{http://localhost:8080} y MySQL en \texttt{mysql://localhost:3306} (usuario/clave \texttt{muebles\_user}/\texttt{muebles\_pass}).
        \item Para detener y limpiar: \texttt{docker compose down -v}.
    \end{enumerate}
    \item \textbf{Ejecución local (sin Docker)}:
    \begin{enumerate}[label=\arabic*.]
        \item Tener MySQL operativo y, si se requiere, sobreescribir credenciales con variables de entorno (\texttt{SPRING\_DATASOURCE\_URL}, \texttt{SPRING\_DATASOURCE\_USERNAME}, \texttt{SPRING\_DATASOURCE\_PASSWORD}).
        \item Levantar la API con datos precargados: \texttt{./mvnw spring-boot:run}. Se expone en el puerto \textbf{8081} (puede cambiarse con \texttt{SERVER\_PORT=8080 ./mvnw spring-boot:run}).
    \end{enumerate}
    \item \textbf{Frontend estático}: el directorio \texttt{frontend/} consume \texttt{/api}. Puede servirse con \texttt{python3 -m http.server --directory frontend 5173} o \texttt{npx serve frontend}, apuntando a \texttt{http://localhost:8081/api} por defecto.
    \item \textbf{Pruebas}: \texttt{./mvnw test} ejecuta JUnit sobre una base H2 aislada; los reportes quedan en \texttt{target/surefire-reports/}.
\end{itemize}
